...
#Some marker gene expression
FeaturePlot(AEC.combined, features = c("Krt5"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Foxj1"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Scgb1a1"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Scgb3a1"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Sftpc"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Bpifb1"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Ptprc"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Col1a2"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Ager"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Aqp5"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Pdpn"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Ascl1"),cols = c("grey","red"), min.cutoff = "q9")
FeaturePlot(AEC.combined, features = c("Calca"),cols = c("grey","red"), min.cutoff = "q9")

AEC.combined.markers <- FindAllMarkers(AEC.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.30)
...
AEC1.combined <- RenameIdents(AEC.combined, `0-AEC2-1` = "0-AEC2",`0-AEC2-2` = "0-AEC2", `0-AEC2-3` = "0-AEC2", `1-AEC1` = "1-AEC1", `2-Basal` = "2-Basal", `3-Club-1` = "3-Club/Goblet", `3-Club-2` = "3-Club/Goblet", `4-Goblet` = "3-Club/Goblet", `6-CCSP+SPC+` = "5-CCSP+SPC+",`5-Ciliated` = "6-Ciliated",  `7-PNEC` = "7-PNEC", `8-Cycling` = "8-Cycling", `9-Fibroblasts` = "8-Fibroblasts", `91-Immune` = "9-Immune", `Unknown` = "Unknown")


DimPlot(AEC.combined, reduction="umap", label=T)

#save rds file for all downstream analysis;
save(AEC.combined, file="AEC2finnalcluster.rds")


#For vlnplot in 3 AEC2 subclusters, use code below;
#change candidate gene to desired gene name;
DefaultAssay(AEC2.cells) <- "RNA"
VlnPlot(object = AEC2.cells, features ="Candidate gene",pt.size =0 , cols = c("red","orange", "blue"))
VlnPlot(object = AEC2.cells, features ="Candidate gene",pt.size =0 , split.by = "group1", cols = c("red","blue"))


#For acitvation score
#download pathway from uniprot;
# TakeLipid biosynthesis Phosphatidylcholine biosynthesis in Figure 6B as an example;
DefaultAssay(AEC2.cells) <- "RNA"
PCB=read.csv("PCB.csv", header=T)
a=as.data.frame(rownames(AEC2.cells))
colnames(a)='Gene'
PCBlist=merge(PCB,a,by="Gene")
PCBs=PCBlist$Gene
PCBscore<- Matrix::colSums(AEC2.cells[PCBs, ])

percent.PCB <- Matrix::colSums(AEC2.cells[PCBs, ])/Matrix::colSums(AEC2.cells)
AEC2.cells$PCBscore=PCBscore
AEC2.cells$percent.PCB=percent.PCB

VlnPlot(object = AEC2.cells, features ="PCBscore", pt.size = 0, cols = c("red","green", "blue"))+ stat_summary(fun.y = mean, geom='errorbar',aes(ymax = ..y.., ymin = ..y..), width = 0.85, linetype = "dashed", size=1.5)+ FontSize(x.text = 30, y.text = 30, x.title = 30,y.title = 30, main = 45)+ NoLegend()
