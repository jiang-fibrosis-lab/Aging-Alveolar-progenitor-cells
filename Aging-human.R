...
plot <- DimPlot(AAAA.combined, reduction = "umap")
select.cells <- CellSelector(plot = plot)
Idents(AAAA.combined, cells = select.cells) <- "CCSP+SPC+"

plot <- DimPlot(AAAA.combined, reduction = "umap")
select1.cells <- CellSelector(plot = plot)
Idents(AAAA.combined, cells = select1.cells) <- "Cycling"

plot <- DimPlot(AAAA.combined, reduction = "umap")
select2.cells <- CellSelector(plot = plot)
Idents(AAAA.combined, cells = select2.cells) <- "PNEC"

plot <- DimPlot(AAAA.combined, reduction = "umap")
select3.cells <- CellSelector(plot = plot)
Idents(AAAA.combined, cells = select3.cells) <- "AEC2"

plot <- DimPlot(AAAA.combined, reduction = "umap")
select4.cells <- CellSelector(plot = plot)
Idents(AAAA.combined, cells = select4.cells) <- "Basal"

save(AAAA.combined, file="AAAAclusterselected.rds")

human.AEC2.cells=subset(AAAA.combined, ident=c("1-AEC2"))
